...
class _BaseRTC:
    ...
    def _register(self, register, buffer=None):
        if buffer is None:
            return self.i2c.mem_read(self.address, register, 1)[0]
        self.i2c.mem_write(self.address, register, buffer)
    ...
    def datetime(self, datetime=None):
        if datetime is None:
            buffer = self.i2c.mem_read(7, self.address, self._DATETIME_REGISTER, timeout=1000)
